...
class Grafo(Grafos):
    ...
    def prim(grafo, vertice_inicial):
        ...
        Retorna a lista de arestas da AGM e a soma total dos pesos.
        """
        ...
        # heap de arestas (peso, origem, destino)
        arestas_candidatas = []

        for aresta in grafo.arestas:
            if aresta["origem"] == vertice_inicial:
                heapq.heappush(arestas_candidatas, (aresta["peso"], aresta["origem"], aresta["destino"]))

        while arestas_candidatas and len(visitados) < len(grafo.grafo_lista):
            peso, origem, destino = heapq.heappop(arestas_candidatas)

            if destino in visitados:
                continue

            # adiciona a aresta na árvore
            agm_arestas.append((origem, destino, peso))
            soma_pesos += peso
            visitados.add(destino)

            # adiciona novas arestas do destino para o heap
            for aresta in grafo.arestas:
                if aresta["origem"] == destino and aresta["destino"] not in visitados:
                    heapq.heappush(arestas_candidatas, (aresta["peso"], aresta["origem"], aresta["destino"]))
        ...
        for origem, destino, peso in agm_arestas:
            print(f"{origem} -- {destino} (peso: {peso})")
        print(f"Soma total dos pesos: {soma_pesos}")
        print(f"Tempo de execução: {tempo_execucao:.6f} segundos")

        return agm_arestas, soma_pesos, tempo_execucao

    def kruskal(self):
        """
        Implementação do algoritmo de Kruskal para árvore geradora mínima
        Retorna: (lista de arestas da AGM, custo total, tempo de execução)
        """
        if not self.ponderado:
            print(f"{RED}Grafo não é ponderado. Kruskal requer grafo ponderado.{RESET}")
            return None, 0, 0

        start_time = time.perf_counter()
        
        # Estrutura para Union-Find (Disjoint Set)
        pai = {v['label']: v['label'] for v in self.grafo_lista}
        
        def encontrar(u):
            while pai[u] != u:
                pai[u] = pai[pai[u]]  # Path compression
                u = pai[u]
            return u
        
        # Prepara lista de arestas únicas (evita duplicatas em grafos não direcionados)
        arestas_unicas = []
        processadas = set()
        
        for aresta in self.arestas:
            chave = frozenset({aresta['origem'], aresta['destino']})
            if chave not in processadas:
                processadas.add(chave)
                arestas_unicas.append({
                    'origem': aresta['origem'],
                    'destino': aresta['destino'],
                    'peso': aresta['peso']
                })
        ...
        arestas_ordenadas = sorted(arestas_unicas, key=lambda x: x['peso'])
        
        agm = []
        custo_total = 0
        
        for aresta in arestas_ordenadas:
            if len(agm) == len(self.grafo_lista) - 1:
                break
            
            raiz_origem = encontrar(aresta['origem'])
            raiz_destino = encontrar(aresta['destino'])
            
            if raiz_origem != raiz_destino:
                agm.append((aresta['origem'], aresta['destino'], aresta['peso']))
                custo_total += aresta['peso']
                pai[raiz_destino] = raiz_origem  # Union
        
        end_time = time.perf_counter()
        tempo_execucao = end_time - start_time
        
        # Exibe resultados
        print(f"\n{PURPLE}=== Árvore Geradora Mínima (Kruskal) ==={RESET}")
        for origem, destino, peso in agm:
            print(f"{origem} -- {destino} (peso: {peso})")
        print(f"{PURPLE}Custo total: {custo_total}{RESET}")
        print(f"{PURPLE}Tempo de execução: {tempo_execucao:.6f} segundos{RESET}")
        
        return agm, custo_total, tempo_execucao